...
void test_binary_to_decimal(void) {
  ...
  // 1111.1 values.
  assert(is_positive_dot("1111.1") == 1);
  text_green("is_positive_dot -> should accept positive binary with dot", 10);
  add_new_line(1);
  assert(is_positive_dot("1111") == 0);
  text_green("is_positive_dot -> should not accept positive binary without dot", 10);
  add_new_line(1);
  assert(is_positive_dot("-1111") == 0);
  text_green("is_positive_dot -> should not accept negative binary without dot", 10);
  add_new_line(1);
  assert(is_positive_dot("-1111.1") == 0);
  text_green("is_positive_dot -> should not accept negative binary with dot", 10);
  add_new_line(1);
